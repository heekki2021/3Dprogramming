...
float trans1_Rmove = 0.0f; //오른쪽 이동 용
...
void Update()
{
    while (!glfwWindowShouldClose(window))
    {
        ...
        trans1_Rmove = trans1_Rmove + 0.001f;
        transform.translate = glm::mat3(
            1, 0, 0,         //1, 0, 0,
            0, 1, 0,                    //0, 1, 0,  
            trans1_Rmove , 0, 1                     //0, 0, 1 
...
;
        ...
        else if (scale1 >= 0.7f && state1 == high)
        {
            scale1 = scale1 - 0.01f;
            if (scale1 < 0.7f)
            {
        ...
        string rmove = to_string(trans1_Rmove);
        cout << rmove << endl;
    }
}
